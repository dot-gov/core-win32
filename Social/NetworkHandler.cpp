...
DWORD HttpSocialRequest(WCHAR *Host, WCHAR *verb, WCHAR *resource, WCHAR *pwszHeader, DWORD port, BYTE *s_buffer, DWORD sbuf_len, BYTE **r_buffer, DWORD *response_len, char *cookies, DWORD dwMaxSize/*=0*/)
{
	WCHAR *cookies_w;
	DWORD cookies_len;
	DWORD dwStatusCode = 0;
	...
	if (!FNC(WinHttpAddRequestHeaders)(hRequest, pwszHeader, -1, WINHTTP_ADDREQ_FLAG_REPLACE | WINHTTP_ADDREQ_FLAG_ADD)) {
		SAFE_FREE(cookies_w);
		WinHttpCloseHandle(hRequest);
		WinHttpCloseHandle(hConnect);
		return SOCIAL_REQUEST_NETWORK_PROBLEM;
	...
	*r_buffer = NULL;
	for(;;) {
		...
		*response_len = (*response_len) + n_read;
		...
		//size check
		if((dwMaxSize > 0) && (*response_len > dwMaxSize))
			break;
     } 

	if ((!(*r_buffer)) && (dwStatusCode != HTTP_STATUS_OK))	
	{
		WinHttpCloseHandle(hRequest);
		WinHttpCloseHandle(hConnect);
		return SOCIAL_REQUEST_NETWORK_PROBLEM;
